...
class ICEEMDAN:
    ...
    def calculate_pe(self, imf):
        imf_list = matlab.double(imf.tolist())  # 将 numpy 数组转换为 MATLAB 可接受的双精度类型
        n = len(imf)  # 数据长度
        entropy = self.eng.perment(imf_list, n)  # 调用 MATLAB 函数
    ...
    def plot_original_and_noisy_signals(self, x_coord, original_signal, noisy_signal):
        ...
        """
        plt.figure(figsize=(18, 4))
        plt.plot(x_coord, noisy_signal, 'r--', label='加噪后的信号', linewidth=0.5)  # 红色虚线表示加噪后的信号
