...
# 识别和合并高斜率窗口的功能
def identify_and_merge_high_slope_windows(data, window_size, step_size, threshold):
    max_slope = 0
...
def calculate_and_save_combined_data(file_path, window_size, step_size, output_dir):
    ...
    # 从第二个程序获取显著点信息
    significant_segments, _ = process_signal(file_path)

    # 获取并合并高斜率窗口
    high_slope_windows = identify_and_merge_high_slope_windows(data, window_size, step_size, HIGH_SLOPE_THRESHOLD)
    ...
    for start in range(0, len(data) - window_size + 1, step_size):
        ...
        combined_data.append([
            ...
            spectral_entropy, spectral_peak_stability, event_start, event_end, event_peak, label#添加新指标
        ])
    ...
    combined_df = pd.DataFrame(combined_data, columns=[
        ...
        'Event Start', 'Event End', 'Event Position', 'Label'
    ])
...
output_directory = r'G:\毕业论文文件\OTDR_Data\特征值文件'  # Set your output directory
latest_file = get_latest_file(DIRECTORY_PATH)
# Ensure that you only get the filename, not the path
latest_filename = os.path.basename(latest_file) if latest_file else None
output_filename = latest_filename.replace('.txt', '_features.csv') if latest_filename else 'output_features.csv'
OUTPUT_CSV = os.path.join(output_directory, output_filename)
calculate_and_save_combined_data(latest_file, WINDOW_SIZE, STEP_SIZE, output_directory)
