...
# 主循环：优化TWSVM参数和特征选择
def optimize_with_bes(X, y, max_epochs=500, pop_size=100, fitness_alpha=0.5):
    ...
    optimizer = BaseBES(problem={
        ...
        "minmax": "min",
    }, epoch=max_epochs,  # 使用传入的 max_epochs
        pop_size=pop_size,  # 使用传入的 pop_size
        a=10,      # 自定义参数 a
        R=1.5,     # 自定义参数 R
        alpha=2, # 自定义参数 alpha
        c1=2,    # 自定义参数 c1
    ...
    for epoch in range(max_epochs):
        ...
        population, current_best = evolve_population(population, X, y, fitness_alpha, beta)
...
# 训练和评估模型
def train_and_evaluate(train_dir, test_dir, fitness_alpha=0.5, max_epochs=500, pop_size=100):
    best_individual = None
...
if __name__ == '__main__':
    ...
    train_and_evaluate(train_dir, test_dir, fitness_alpha=0.5, max_epochs=500, pop_size=100)
