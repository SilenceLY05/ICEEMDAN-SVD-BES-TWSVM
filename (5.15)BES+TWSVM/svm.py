...
class svm:
    ...
    def __init__(self, kernel = 'rbf'):
        ...
        # linear : linear   线性
        # poly   : polynomial   多项式
        # rbf    : radial base gaussian function (RFB)    径向基高斯函数
        # erbf   : radial base exponencial    径向底指数
        # tanh   : hyperbolic tangent    双曲正切
        # lspline: linear splines    线性样条
        return
    ...
    def train(self, X_train, y_train, model):
        ...
        if y_train.shape[1] > 2:
            print('Only binary models, for multi-class use svm-mc')   #如果标签大于两列是多分类任务，不进行进一步操作
            return
    ...
    # ==================================
    def predict(self, X_test, X_train = None, y_train = None):   #用于预测新数据，根据初始化设置的模型类型，调用相应的预测方法
        """
        Predict new values.
    ...
    # 1.1 SVM: Predict
    # ==================================
    def svm_predict(self, X_test, X_train, y_train):   #用标准SVM模型对测试数据进行预测。这个方法考虑了支持向量和拉格朗日乘子来进行决策
        """
        Predict SVM.
    ...
    # 1.2 SVM: Train
    # ==================================
    def svm_fit(self, X_train, y_train):  #用标准SVM模型拟合训练数据。根据定义的核函数和软边缘参数，使用 cvxopt 库解决二次规划问题。
        """
    ...
    # 2.1 LSSVM: Predict
    # ==================================
    def lssvm_predict(self, X_test, X_train, y_train):  #使用最小二乘支持向量机（LSSVM）进行预测。
        """
    ...
    # 2.2 LSSVM: Train
    # ==================================
    def lssvm_fit(self, X_train, y_train):   #使用最小二乘支持向量机（LSSVM）拟合训练数据。这种方法通过解线性方程来获取拉格朗日乘子。
        """
    ...
    # ==================================
    # Paper documentation
    def twsvm_predict(self, X_test):    #基础的TWSVM预测
        """
    ...
    def twsvm_predict_clodoaldo(self, X_test):   #优化后的TWSVM预测，处理了样本正好位于两个平面之间的情况
        """
        Predict TW-SVM values
        Input
            X_test:         test set. Numeric normalized values or categorical values
    ...
    def twsvm_fit(self, X_train, y_train):     #使用双支持向量机拟合训练数据。这种方法解决了两个二次规划问题来寻找决策平面。
        """
    ...
    def svm_mc(self, X_train, y_train, X_test, y_test):    #处理多类分类问题的方法，支持一对一（ovo）和一对所有（ova）策略。model在初始化中设置
        """
        Input
            X_train:        train set. Numeric normalized values or categorical values
                            encoded as numeric.
        ...
        Output
            Trained model stored in svm object.
        """
        if type(y_train) is not np.ndarray:    #确保 y_train 是 NumPy 数组类型，以便后续操作。
        ...
        if self.method == 'ovo':
            ...
            N, nc = y_test.shape    #将每对类别分别训练一个二分类器。N 是测试集样本数，nc 是类别数。
            ...
            X2 = X_test
            k = 0
            for i in range(nc):    #遍历所有可能的类别对，其中 i 是正类，j 是负类。
                for j in range(i + 1, nc):
                    # Create the train and test sets
                    # X1, y1: train sets
                    # X2, y2: test  sets
                    pos = y_train[:, i] == 1  # index of positive class    #从训练集中选出当前正类和负类的样本和标签，用于训练一个针对这两类的 SVM 模型
                    ...
                    # Train and predict
                    self.train(X1, y1, self.model)            # Train    #使用提取的训练数据来训练模型，然后对整个测试集 X2 进行预测。
                    y = self.predict(X2, X1, y1)  # Predict

                    # Keep predictions in a matrix          #初始化或更新一个临时的预测结果数组，用于存储每个二分类器的预测结果。
                    if k == 0:
                        y_hat_temp = y
                    else:
            ...
            y_hat = np.zeros((N, nc))              #统计每个样本对每个类的投票，然后选择获得最多票数的类别作为最终预测结果。
            for n in range(N):
                k = 0
                for i in range(nc):
                    for j in range(i + 1, nc):
        ...
        # One vs All
        # ---------------
        elif self.method == 'ova':                #一对所有策略 统计每个样本对每个类的投票，然后选择获得最多票数的类别作为最终预测结果。
            ...
            X2 = X_test
            acc = np.ones((nc))
            for i in range(nc):                       #为每个类别训练一个模型并进行预测，然后将预测结果存储在 y_hat 数组中。
                # Create the train and test sets
                # X1, y1: train
                # X2, y2: test
                y1 = y_train[:, i:i+1]
        ...
        y_hat = np.where(y_hat == -1, 0, y_hat)        #这行代码处理预测结果，将所有 -1 转换为 0（如果有必要），并返回最终的预测结果数组
        return y_hat
    ...
    def tune_rbf(self, X_train, y_train, X_test, y_test, K=5, plot=False, pdir = '', DS_name=''):  #通过交叉验证和网格搜索调整径向基函数（RBF）核的参数，寻找最优的参数配置。
        # Input
    ...
    def multi_svm(self, X_train, y_train, X_test, y_test, DS_name, results, tune=True, plot = False, pdir = ''):
        # 该方法用于自动化地训练和测试多种 SVM 相关模型（包括标准 SVM、LSSVM 和 TWSVM），支持不同的核类型，并且可选是否进行 RBF 核的参数调优。
        # Input
    ...
    def multi_mc(self, X_train, y_train, X_test, y_test, results, tune=True,
        ...
        #与 multi_svm 类似，但专门用于多类分类情况，支持 SVM、LSSVM 和 TWSVM，同时考虑不同的多类策略（ovo 和 ova）。
        # DS_name: dataset name, e.g. 'Diabetes'
        # results: dataframe with test results
        # ----------------------------
