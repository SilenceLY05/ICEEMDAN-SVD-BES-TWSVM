...
class BaseBES(Optimizer):
    ...
    def __init__(self, problem, epoch=500, pop_size=100, a=10, R=1.5, alpha=2, c1=2, c2=2, **kwargs):
        """
        Args:
            problem ():
    ...
    def evolve(self, epoch):
        ...
        ## 1. Select space
        pos_list = np.array([individual[self.ID_POS] for individual in self.pop])  #从当前种群self.pop中提取每个个体的位置self.ID_POS并生成一个数组
        pos_mean = np.mean(pos_list, axis=0)   #当前所有个体位置的平均值

        pop_new = []  #初始化一个新的种群列表
        for idx in range(0, self.pop_size):   #对种群内的每个个体操作
            pos_new = self.g_best[self.ID_POS] + self.alpha * np.random.uniform() * (pos_mean - self.pop[idx][self.ID_POS])  #计算新的pos_new
            #self.g_best[self.ID_POS]: 当前最佳个体的位置。 self.alpha * np.random.uniform(): 控制变化的参数，乘以一个随机数。 (pos_mean - self.pop[idx][self.ID_POS]): 当前个体与种群平均位置的差。
            pos_new = self.amend_position_faster(pos_new)  #调用 amend_position_faster 函数来修正新的位置，使其在允许范围内。
            pop_new.append([pos_new, None])  #将新的位置添加到 pop_new 列表中，初始适应度值为 None。
        pop_new = self.update_fitness_population(pop_new)   #更新 pop_new 中个体的适应度值。
