...
class BaseCABES(Optimizer):
    ...
    def _cauchy_mutation(self, x):
        return x + np.random.standard_cauchy(x.shape)

    def _adaptive_weight(self, epoch):
        return np.sin(np.pi * epoch / (2 * self.epoch) + np.pi) + 1
    ...
    def evolve(self, epoch):
        ...
        for idx in range(0, self.pop_size):
            ...
            pos_new = self._cauchy_mutation(pos_new)  # Apply Cauchy mutation
        ...
        pos_mean = np.mean(pos_list, axis=0)
        adaptive_weight = self._adaptive_weight(epoch)  # Calculate adaptive weight
        ...
        for idx in range(0, self.pop_size):
            ...
            pos_new = adaptive_weight * pos_new  # Apply adaptive weight
            pos_new = self.amend_position_faster(pos_new)
