...
class BaseMBES(Optimizer):
    ...
    def _create_x_y_x1_y1_(self,epoch):
        ...
        ## Eq. 2
        C1 = self.a * (1 + (self.epoch - epoch) / (self.epoch + epoch))  # Adaptive C1 parameter for second phase
        theta = C1 * np.pi * np.random.rand(self.pop_size)
        r = theta + self.R * np.random.uniform(0, 1, self.pop_size)
        xr, yr = r * np.sin(theta), r * np.cos(theta)
    ...
    def evolve(self, epoch):
        ...
        TF = self.alpha * (1 + (self.epoch - epoch) / (self.epoch + epoch))  # Adaptive transition factor

        pop_new = []
        for idx in range(0, self.pop_size):
            pos_new = self.g_best[self.ID_POS] + TF * np.random.uniform() * (pos_mean - self.pop[idx][self.ID_POS])
        ...
        pos_mean = np.mean(pos_list, axis=0)
        EO = 2 + np.sin(2.5 + epoch / self.epoch)  # Enhancement operator

        pop_new = []
        for idx in range(0, self.pop_size):
            pos_new = np.random.uniform() * self.g_best[self.ID_POS] + x1_list[idx] * (pop_child[idx][self.ID_POS] - self.c1 * pos_mean) \
                      + y1_list[idx] * (pop_child[idx][self.ID_POS] - EO * self.g_best[self.ID_POS])
            pos_new = self.amend_position_faster(pos_new)
            pop_new.append([pos_new, None])
        pop_new = self.update_fitness_population(pop_new)
