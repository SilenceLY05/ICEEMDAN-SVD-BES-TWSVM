...
def select_space(fobj, pop, npop, BestSol, low, high, dim, t, MaxIt):
    ...
    for i in range(npop):
        ...
        TF = lm * (1 + (MaxIt - t) / (MaxIt + t))
        newsol_pos = BestSol['pos'] + TF * np.random.rand(dim) * (Mean - pop['pos'][i, :])
...
def swoop(fobj, pop, BestSol, npop, low, high, t, MaxIt):
    Mean = np.mean(pop['pos'], axis=0)
    a = 10
    R = 1.5
    s1 = 0
    ...
    for i in range(npop):
        x, y = swoo_p(a, R, npop)
        EO = 2 + np.sin( 2.5 + t / MaxIt)
        Step = pop['pos'][i, :] - 2 * Mean
        Step1 = pop['pos'][i, :] - EO * BestSol['pos']
