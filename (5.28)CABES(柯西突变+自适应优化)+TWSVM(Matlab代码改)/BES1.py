...
def select_space(fobj, pop, npop, BestSol, low, high, dim):
    ...
    for i in range(npop):
        ...
        C = np.random.standard_cauchy(dim)  # 生成柯西分布的随机数
        C = np.clip(C, -10, 10)  # 裁剪柯西分布的随机数到[-10, 10]
        newsol_pos = BestSol['pos'] + C * np.random.rand(dim) * (Mean - pop['pos'][i, :])
...
def search_space(fobj, pop, BestSol, npop, low, high, t, MaxIt):
    Mean = np.mean(pop['pos'], axis=0)
    a = 10
    R = 1.5
    s1 = 0
    ...
    for i in range(npop - 1):
        ...
        omiga = np.sin((np.pi * t) /( 2* MaxIt) + np.pi) + 1   #引入自适应权重因子
        newsol_pos = newsol_pos * omiga
