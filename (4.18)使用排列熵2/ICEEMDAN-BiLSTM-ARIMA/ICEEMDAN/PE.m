...
% @email UnakafovaValentina(at)gmail.com
 ...
 indices = double(1:windowSize); peTable = -(indices) .* log(double(indices)); % table of precomputed values
 if windowSize > 1  % 仅当 windowSize 大于 1 时执行，以避免索引错误
    indices2toWindowSize = double(2:windowSize);
    indices1toWindowSizeMinus1 = double(1:windowSize - 1);

    % 更新 peTable 中的值
    peTable(indices2toWindowSize) = double((peTable(indices2toWindowSize) - peTable(indices1toWindowSizeMinus1)) ./ double(windowSize));
end
 for iTau = 1:delay 
     cnt = iTau; 
 ...
 ordDistNorm = double(patternsDistribution)/double(windowSize);
 ...
 ePE = ePE( windowSize + delay*order:end )/double(order);