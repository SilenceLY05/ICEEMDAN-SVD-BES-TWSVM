...
from tkinter_config import set_chinese_font
...
class ICEEMDAN:
    ...
    def calculate_pe(self,imf,delay,order,window_size):
        imf_list = matlab.double(imf.tolist())
        entropy = self.eng.PE(imf_list,delay,order,window_size)
        return np.array(entropy).flatten()[0]
    ...
    def iceemdan_and_denoise(self, original_signal, df):
        ...
        for i, imf in enumerate(imfs):
            # 使用排列熵
            delay = 6  # 排列熵的嵌入维度，可调整
            order = 1  # 排列熵的嵌入延迟，可调整
            window_size = 3000

            entropy = self.calculate_pe(imf,delay,order,window_size)
            print(f"IMF {i + 1}: Permutation Entropy = {entropy:.4f}")  # 输出每个IMF的排列熵值
            if entropy >= self.entropy_threshold:
                print(f"IMF {i + 1} is considered noisy due to high entropy.")
                # 进行去噪处理
                #denoised_imf = self.denoiser.denoise(imf)
                # 再次计算样本熵
                #entropy_after_denoise = permutation_entropy(denoised_imf, embdim, embdelay)
    ...
    def add_noise(self, signal, x_coord, noise_level_base=0.05, noise_level_after=0.2, threshold=26.3):
        """
        在原始信号中加入噪声。
        :param signal: 原始信号数组。
...
if __name__ == "__main__":
    ...
    entropy_threshold = 0.7 # 这是一个示例值，您需要根据自己的需求来确定合适的阈值
