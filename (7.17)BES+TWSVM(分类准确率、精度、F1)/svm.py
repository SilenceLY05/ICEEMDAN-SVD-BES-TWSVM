...
class svm:
    ...
    def __init__(self, kernel = 'rbf'):
        self.model = 'twsvm'          # 'svm', 'lssvm', 'twsvm'
    ...
    # ==================================
    # 0.1 Train any model (SVM, LSSVM, TWSVM)   训练方法 输入：x_train 训练数据的特征 y_train 训练数据的标签   model指定要训练的模型类型
    # ==================================
    ...
    # ==================================
    # 0.2 Predict any model (SVM, LSSVM, TWSVM)  预测方法用于预测新数据的标签
    # ==================================
    def predict(self, X_test, X_train = None, y_train = None):
        ...
        Input
            X_test:         test set. Numeric normalized values or categorical values   测试数据的特征
                            encoded as numeric.
            X_train:        train set. Numeric normalized values or categorical values  可选 用于SVM在预测时参考的训练数据集
                            encoded as numeric.
            y_train:        train set, y={-1, 1} or y={0, 1}. Vector if binary,   可选，用于SVM在预测时参考的训练数据集
                            one-hot enconded if multiclass.
    ...
    # 0.3 Kernel
    # ==================================
    # Compute kernel matrix k(x1, x2)    内核函数 计算两个数据集之间的内核矩阵
    ...
    def svm_predict(self, X_test, X_train, y_train):
        ...
        # Compute the bias
        k = self.kernel(X_train.astype(float), X_test.astype(float))
        SV_neg = y_train < 0
        SV_pos = y_train > 0
    ...
    def svm_fit(self, X_train, y_train):
        ...
        A = cvxopt.matrix(y_train.astype(float).reshape(1, -1), tc='d')
    ...
    def lssvm_predict(self, X_test, X_train, y_train):
        ...
        y_hat = np.sign(np.sum(self.alpha * y_train * self.kernel(X_train.astype(float), X_test.astype(float)), axis=0,
    ...
    def lssvm_fit(self, X_train, y_train):
        ...
        N = X_train.shape[0]
        nc = y_train.shape[1]
        K = self.kernel(X_train.astype(float), X_train.astype(float))
        ...
        # 4.2 Output vector b
        b = np.vstack((np.zeros((1, 1)), np.ones((N, 1)))).astype(float)
    ...
    def svm_mc(self, X_train, y_train, X_test, y_test):
        ...
        y_hat_labels = y_hat.argmax(axis=1)

        # Compute True Positives (TP), False Positives (FP), False Negatives (FN) for each class
        classes = np.unique(y_test_labels)
        tp = {cls: 0 for cls in classes}
        fp = {cls: 0 for cls in classes}
        fn = {cls: 0 for cls in classes}

        for true_label, pred_label in zip(y_test_labels, y_hat_labels):
        ...
        weighted_precision = sum(precision_per_class[cls] * tp[cls] for cls in classes) / total_tp if total_tp > 0 else 0
        weighted_recall = sum(recall_per_class[cls] * tp[cls] for cls in classes) / total_tp if total_tp > 0 else 0

        # Compute F1 score
        f1_score_manual = 2 * (weighted_precision * weighted_recall) / (weighted_precision + weighted_recall) if (weighted_precision + weighted_recall) > 0 else 0

        # Compute accuracy
        accuracy = total_tp / len(y_test_labels)

        # Classification report for detailed metrics
        report = classification_report(y_test_labels, y_hat_labels, output_dict=True)

        # Compute misclassification details
        misclassified = np.where(y_test_labels != y_hat_labels)
        misclassification_details = [(y_test_labels[i], y_hat_labels[i]) for i in misclassified[0]]

        return y_hat, precision_per_class, recall_per_class, f1_score_manual, accuracy, report, misclassification_details
    ...
    def tune_rbf(self, X_train, y_train, X_test, y_test, K=5, plot=False, pdir = r'G:\OTDR_Data', DS_name='twsvm'):
        # Input
        ...
        # Values to be tuned
        C_vals = [2 ** 15]
        gamma_vals = [2 ** -11, 2 ** -9, 2 ** -7, 2 ** -5, 2 ** -3, 2 ** -1, 2 ** 1, 2 ** 3]
        # ----------------------------
            ...
            for C in C_vals:
                ...
                for gamma in gamma_vals:
                    ...
                    for k in range(K):
                        ...
                        # Compute the accuracy
                        acc = (100 * np.mean(y_hat == y2))

                        print(f"svm Iteration {k + 1}/{K}, C: {C}, Gamma: {gamma}, Accuracy: {acc}%")
...
if __name__ == '__main__':
    ...
    def load_dataset(folder_path):
        ...
        X = data.iloc[:, 2:32].values.astype(float)
        y = data.iloc[:, 33].values
        # Filter out noise labels 0 and 6
        valid_indices = (y != 0) & (y != 6)
        X = X[valid_indices]
        y = y[valid_indices]
        # Convert labels 1-5 to one-hot encoding
        y = pd.get_dummies(y).values
        return X, y

    # Load training and test datasets
    train_folder_path = r'G:\OTDR_Data\特征值文件 - 副本'
    test_folder_path = r'G:\OTDR_Data\测试集 - 副本'
    ...
    fit.sigma = 0.125               # RBF and ERBF sigma
    # Twin SVM settings
    ...
    print('twsvm Accuracy:', acc)
    print('twsvm time:', timep)
    """
    ...
    fit.model = model
    start_time = time.time()
    y_hat, precision_per_class, recall_per_class, f1_score_manual, accuracy, report, misclassification_details = fit.svm_mc(
    ...
    print('Misclassification Details:', misclassification_details)
    print('TW-SVM Time:', timep)

    """
    model = 'lssvm'  # 'svm', 'lssvm', 'twsvm'
    ...
    timep = end_time - start_time
    acc = np.mean(y_hat == y_test)
    print('lssvm Accuracy:', acc)
    print('lssvm time:', timep)
    ...
    # ----------------------------
    start_time = time.time()
    fit.model = 'twsvm'   # possible values: 'svm', 'lssvm', 'twsvm'
    y_hat = fit.tune_rbf(X_train, y_train, X_test, y_test, K = 5, plot = True,
                         pdir = r'G:\OTDR_Data', DS_name = 'twxvm')
    acc = np.mean(y_hat == y_test)
    print('Tuned RBF kernel Accuracy:', acc)
    end_time = time.time()
    timep = end_time - start_time
    print('Tune RBF kernel time:', timep)
