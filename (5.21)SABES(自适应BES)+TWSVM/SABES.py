...
class BaseSABES(Optimizer):
    ...
    def __init__(self, problem, epoch=10000, pop_size=100, a=10, R=1.5, alpha=2, c1=2, c2=2, **kwargs):
        ...
        # Initialize population using equation (7)
        self.pop = [self.create_solution() for _ in range(self.pop_size)]

    def create_solution(self):
        solution = np.random.uniform(self.problem.lb, self.problem.ub)
        return [solution, None]

    def dynamic_opposite(self, pos, lb, ub):
        S_O = lb + ub - pos
        return pos + np.random.uniform() * (np.random.uniform() * S_O - pos)

    def _create_x_y_x1_y1_(self, epoch):
        t_max = self.epoch
        ...
        x_list = xr / np.max(np.abs(xr))
        y_list = yr / np.max(np.abs(yr))
        x1_list = xr1 / np.max(np.abs(xr1))
        y1_list = yr1 / np.max(np.abs(yr1))

        a_min, a_max = 5, 10
        R_min, R_max = 0.5, 2
        alpha_min, alpha_max = 1.5, 2
        n = 1.2

        self.a = ((t_max - epoch) / t_max) ** n * (a_max - a_min) + a_min
        self.R = ((t_max - epoch) / t_max) ** n * (R_max - R_min) + R_min
        self.alpha = ((t_max - epoch) / t_max) ** n * (alpha_max - alpha_min) + alpha_min

        return x_list, y_list, x1_list, y1_list

    def evolve(self, epoch):
        t_max = self.epoch
        x_list, y_list, x1_list, y1_list = self._create_x_y_x1_y1_(epoch)

        # Initialization with Dynamic-Opposite Learning
        for individual in self.pop:
            individual[self.ID_POS] = self.dynamic_opposite(individual[self.ID_POS], self.problem.lb, self.problem.ub)

        # Calculate fitness of initial population
        self.pop = self.update_fitness_population(self.pop)
        self.pop = sorted(self.pop, key=lambda x: x[self.ID_FIT])
        ...
        # Update fitness values and best search agent
        self.pop = pop_new
        self.g_best = min(self.pop, key=lambda x: x[self.ID_FIT])
        ...
        pop_child = []
        for idx in range(self.pop_size):
            idx_rand = np.random.choice(list(set(range(self.pop_size)) - {idx}))
            ...
            pos_new = self.dynamic_opposite(pos_new, self.problem.lb, self.problem.ub)
            pos_new = self.amend_position_faster(pos_new)
            pop_child.append([pos_new, None])
        pop_child = self.update_fitness_population(pop_child)
        pop_child = self.greedy_selection_population(self.pop, pop_child)

        # Update best search agent
        self.pop = pop_child
        self.g_best = min(self.pop, key=lambda x: x[self.ID_FIT])
        ...
        self.a = ((t_max - epoch) / t_max) * (a_max - a_min) + a_min
        self.c1 = ((t_max - epoch) / t_max) * (c1_max - c1_min) + c1_min
        self.c2 = ((t_max - epoch) / t_max) * (c2_max - c2_min) + c2_min
