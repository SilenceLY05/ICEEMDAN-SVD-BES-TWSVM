...
class TWSVMWrapper(BaseEstimator, ClassifierMixin):
    def __init__(self, c1=1, c2=1, sigma=1):
        self.c1 = c1
        self.c2 = c2
        self.sigma = sigma
        self.model = None
    ...
    def fit(self, X, y):
        self.model = svm()
        self.model.model = 'twsvm'
        self.model.c1 = self.c1
        self.model.c2 = self.c2
        self.model.sigma = self.sigma
        self.model.train(X, y, 'twsvm')
        return self
...
def calculate_fitness(individual, X_train, y_train, alpha=0.99):
    ...
    model = TWSVMWrapper(c1=c1, c2=c2, sigma=gamma)

    # 使用交叉验证评估分类器性能
    scores = cross_val_score(model, X_train[:, selected_features], y_train, cv=5, scoring='accuracy')
    acc = np.mean(scores)
...
def train_and_evaluate(train_folder, test_folder, nPop=100, MaxIt=500):
    ...
    for file in test_files:
        ...
        fit = svm()
        fit.model = 'twsvm'
        fit.method = 'ovo'  # 确保使用 One vs One 方法
...
if __name__ == "__main__":
    train_folder = r"G:\OTDR_Data\特征值文件 - 副本"
    test_folder = r"G:\OTDR_Data\测试集 - 副本"
