...
from BES1 import initialize_population, select_space, search_space, swoop, leader_based_mutation_selection
from svm import svm
...
def calculate_fitness(individual, X_train, y_train, alpha=0.99):
    ...
    for train_index, test_index in kf.split(X_train):
        X_tr, X_val = X_train[train_index][:, selected_features], X_train[test_index][:, selected_features]
        y_tr, y_val = y_train[train_index], y_train[test_index]
        y_hat = fit.svm_mc(X_tr, y_tr, X_val, y_val)

        # 确保 y_hat 和 y_val 是相同的格式
        y_hat = np.array(y_hat).reshape(y_val.shape)
...
def evolve_population(pop, BestSol, nPop, low, high, dim, X_train, y_train, MaxIt):
    ...
    for t in range(MaxIt):
        pop, BestSol, s1 = select_space(lambda ind: calculate_fitness(ind, X_train, y_train), pop, nPop, BestSol, low, high, dim, t, MaxIt)
        ...
        pop, BestSol, s4 = leader_based_mutation_selection(lambda ind: calculate_fitness(ind, X_train, y_train), pop, BestSol, nPop, low, high, t, MaxIt)
