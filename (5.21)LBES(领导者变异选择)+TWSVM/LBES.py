...
class BaseLBES(Optimizer):
    ...
    def __init__(self, problem, epoch=10000, pop_size=100, a=10, R=1.5, c1=2, c2=2, **kwargs):
        """
        Args:
            problem ():
    ...
    def evolve(self, epoch):
        ...
        ## 1. Select space
        alpha = 1.5 * (self.epoch - epoch + 1) / self.epoch  # Updated alpha calculation
        pos_list = np.array([individual[self.ID_POS] for individual in self.pop])
        pos_mean = np.mean(pos_list, axis=0)

        pop_new = []
        for idx in range(0, self.pop_size):
            pos_new = self.g_best[self.ID_POS] + alpha * np.random.uniform() * (pos_mean - self.pop[idx][self.ID_POS])
        ...
        for idx in range(0, self.pop_size):
            ...
            pos_new = pop_new[idx][self.ID_POS] + y_list[idx] * (
                        pop_new[idx][self.ID_POS] - pop_new[idx_rand][self.ID_POS]) + \
