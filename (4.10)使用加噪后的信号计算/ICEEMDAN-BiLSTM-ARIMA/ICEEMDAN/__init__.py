...
class ICEEMDAN:
    ...
    def plot_signals(self, noisy_signal, reconstructed_signal):
        ...
        plt.plot(noisy_signal, 'b', label='原始信号')  # 蓝色表示原始信号
    ...
    def plot_original_and_noisy_signals(self, original_signal, noisy_signal):
        ...
        """
        plt.figure(figsize=(18, 4))
        plt.plot(noisy_signal, 'r--', label='加噪信号')  # 红色虚线表示加噪后的信号
        plt.plot(original_signal, 'g', label='原始信号')  # 绿色表示原始信号
    ...
    def process_folder(self):
        for file in os.listdir(self.input_folder):
            if file.endswith(".txt"):  # Check for .txt file extension
                ...
                noisy_signal = self.add_noise(original_signal)
