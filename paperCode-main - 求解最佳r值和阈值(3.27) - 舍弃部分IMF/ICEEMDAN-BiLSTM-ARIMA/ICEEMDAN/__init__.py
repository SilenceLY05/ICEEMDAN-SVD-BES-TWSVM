    ...
    def iceemdan_and_denoise(self, df, r, entropy_threshold):
        ...
        for i, imf in enumerate(imfs):
            ...
            if entropy >= entropy_threshold:
                ...
                # 再次计算样本熵
                entropy_after_denoise = self.sampen_calculator.sampen(denoised_imf, 2, r_dynamic)
                if entropy_after_denoise >= entropy_threshold:
                    print(f"IMF {i + 1} remains noisy after denoising and will be discarded.")
                    denoised_imfs[i] = np.zeros_like(imf)  # 将这个IMF舍弃
                else:
