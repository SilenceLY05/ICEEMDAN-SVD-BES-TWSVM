...
def evolve_population(pop, BestSol, nPop, low, high, dim, X_train, y_train, MaxIt):
    ...
    beta = 1.2
    theta = beta * np.arccos(2 * rand - 1)

    for t in range(MaxIt):
        pop, BestSol, s1 = select_space(lambda ind: calculate_fitness(ind, X_train, y_train), pop, nPop, BestSol, low, high, dim)
        pop, BestSol, s2 = search_space(lambda ind: calculate_fitness(ind, X_train, y_train), pop, BestSol, nPop, low, high, theta)
        previous_best_cost = BestSol['cost']  # 保存上一轮swoop更新后的BestSol
        pop, BestSol, s3 = swoop(lambda ind: calculate_fitness(ind, X_train, y_train), pop, BestSol, nPop,  low, high, theta)
        Convergence_curve[t] = BestSol['cost']
        print(f"Iteration {t + 1}, Best Cost: {BestSol['cost']}")

        # Update theta based on whether BestSol is improved in the swoop stage compared to previous swoop stage
        if BestSol['cost'] < previous_best_cost:
            theta = beta * theta + 2 * np.arccos(2 * np.random.rand() - 1)
        else:
            theta = beta * theta - 2 * np.arccos(2 * np.random.rand() - 1)
...
def train_and_evaluate(train_folder, test_folder, nPop=100, MaxIt=500):
    ...
    for file in train_files:
        X_train, y_train, _, _, _, _ = load_features_from_file(file, is_train=True)
