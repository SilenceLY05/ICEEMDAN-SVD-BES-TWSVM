...
class ICEEMDAN:
    ...
    def iceemdan_and_denoise(self, original_signal, df ,x_coord):
        ...
        rmse = calculate_rmse(original_signal, denoised_signal)
    ...
    def process_folder(self):
        for file in os.listdir(self.input_folder):
            if file.endswith(".txt"):  # Check for .txt file extension
                ...
                # 去噪并重构信号
                # 去噪并重构信号
                denoised_signal, imfs, valid_imfs, snr, rmse = self.iceemdan_and_denoise(
                    original_signal, noisy_signal, x_coord)

                # 使用修改后的方法绘制原始信号和重构信号
                self.plot_signals(x_coord, noisy_signal, denoised_signal)

                # 创建一个新的 DataFrame 来保存重构的信号和距离信息
                combined_df = pd.DataFrame({
                    'Distance': x_coord,  # 假设第一列是距离信息
                    'Amplitude': denoised_signal
                })
